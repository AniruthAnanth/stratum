...
//!
//! Unlike its macOS counterpart this test is not `cfg`-gated, and neither is
//! the scan: it reads every `.rs` file under `src/` from disk regardless of
//! which of them this host compiles, so the D-Bus and syscall modules are
//! covered from a machine that cannot build them.
...
fn no_shipped_path_in_this_crate_can_abort() {
    ...
    // Every module, whether or not this host compiles it. Cheap insurance
    // against a `cfg`-gated file drifting because nobody's compiler sees it.
    assert!(
        files.len() >= 14,
        "expected every module to be scanned, found {}",
        files.len()
    );

    // The `#[cfg(test)]` modules inside `src/` are deliberately NOT exempt.
    // Every one of them is written with `assert!`/`assert_eq!`/`is_some_and`
    // rather than `unwrap`, which costs a few characters and means this scan
    // needs no exception anyone can widen later.
